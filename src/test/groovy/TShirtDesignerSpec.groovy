...
class TShirtDesignerSpec extends GebReportingSpec {
    ...
    def "The should calculate the printing cost"(){
        ...
        when: "i request the product url api"
        def bodyString = '{"data":{"product":{"id":"n50p-na"},"locations":[{"name":"Front","colors":0,"isFullColor":false,"colorsNum":0,"colorsList":[],"objects":[{"type":"image","isUploaded":false,"id":"first-image-sam","colorsList":[],"colorsNum":0,"colors":0,"isFullColor":false}],"objectCount":1,"letterings":0,"images":1},{"name":"Back","colors":0,"isFullColor":false,"colorsNum":0,"colorsList":[],"objects":[],"objectCount":0,"letterings":0,"images":0}],"colorsList":[],"colors":0,"isFullColor":false,"colorsNum":0,"quantities":[{"size":"S","quantity":1}],"namesNumbers":[]}}'
        def jsonObj = new JsonSlurper().parseText(bodyString)
        def resp = root.post("customizations/get_quote", jsonObj)

        then: "the  products are grouped by categories"
        assert resp['prices'].size > 0
    }
}
